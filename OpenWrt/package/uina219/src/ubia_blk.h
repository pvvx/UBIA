...
// CMD_DEV_VER Get Ver
typedef struct  __attribute__((packed)) _dev_get_ver_t{
	uint16_t id;
	uint16_t ver;
} dev_get_ver_t;
...
typedef struct __attribute__((packed)) _i2c_utr_t {
	uint8_t	mode;	// bit0..6: number wr_byte for new START (bit7: =1 - generate STOP/START)
	uint8_t	rdlen; // bit7: =1 - old read byte generate NACK, =0 - ACK
	uint8_t	wrdata[1]; // i2c_addr_wr, wr_byte1, wr_byte2, wr_byte3, ... wr_byte126
} i2c_utr_t;
